...
export const CumulativeSampleMean: React.FC<CumulativeSampleMeanProps>  = (
    ...
    useEffect(() => {
        ...
        // Calculate min and max values for prevData
        let minYPrevData = minYValue;
        let maxYPrevData = maxYValue;
        if (prevData.length > 0) {
            prevData.forEach((oldData) => {
                const minY = min(oldData, (d) => d[1]);
                const maxY = max(oldData, (d) => d[1]);
                minYPrevData = Math.min(minYPrevData, minY);
                maxYPrevData = Math.max(maxYPrevData, maxY);
            });
        }

        // Calculate min and max values for prevData2
        if (prevData2.length > 0) {
        ...
        const overallMinY = Math.min(minYValue, minYPrevData);
        const overallMaxY = Math.max(maxYValue, maxYPrevData);

        // Calculate the y-axis tick values
        const tickValues = range(
            overallMinY, overallMaxY,
            (overallMaxY - overallMinY) / (numTicks - 1));
